...
{
	public class AircraftG2Info : ITraitInfo, IPositionableInfo, IFacingInfo, IMoveInfo, ICruiseAltitudeInfo,
		IActorPreviewInitInfo, IEditorActorOptions
	{
		...
		public virtual object Create(ActorInitializer init) { return new AircraftG2(init, this); }
	...
	public class AircraftG2 : ITick, ISync, IFacing, IPositionable, IMove, IIssueOrder, IResolveOrder, IOrderVoice, IDeathActorInitModifier,
		...
		IActorPreviewInitModifier, IIssueDeployOrder, IObservesVariables
	{
		...
		public readonly AircraftG2Info Info;
		readonly Actor self;
		...
		public AircraftG2(ActorInitializer init, AircraftG2Info info)
		{
			Info = info;
			self = init.Self;
		...
		protected virtual void Created(Actor self)
		{
			...
			attackMove = self.TraitOrDefault<AttackMoveG2>();
		...
		protected virtual void Tick(Actor self)
		{
			if (firstTick)
			{
				...
				// TODO: AircraftG2 husks don't properly unreserve.
		...
		public virtual WVec GetRepulsionForce()
		{
			...
			foreach (var actor in self.World.FindActorsInCircle(self.CenterPosition, Info.IdealSeparation))
			{
				...
				var ai = actor.Info.TraitInfoOrDefault<AircraftG2Info>();
		...
		{
			// Map.DistanceAboveTerrain(WPos pos) is called directly because AircraftG2 is an IPositionable trait
		...
		{
			UnReserve();
			var reservable = target.TraitOrDefault<ReservableG2>();
			if (reservable != null)
			{
		...
		public IEnumerable<IOrderTargeter> Orders
		{
			get
			{
				...
				yield return new AircraftG2MoveOrderTargeter(Info);
			}
		}
		...
		public void ResolveOrder(Actor self, Order order)
		{
			if (order.OrderString == "Move")
			{
				...
				if (!Info.CanHover)
					...
					self.QueueActivity(order.Queued, new HeliFlyAndLandWhenIdleG2(self, target, Info));
			}
