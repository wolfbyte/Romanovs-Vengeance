...
namespace OpenRA.Mods.RA2.Traits
{
	...
	class AttackMoveG2Info : ITraitInfo, Requires<IMoveInfo>
	{
		...
		public object Create(ActorInitializer init) { return new AttackMoveG2(init.Self, this); }
	}

	class AttackMoveG2 : INotifyCreated, ITick, IResolveOrder, IOrderVoice, INotifyIdle, ISync
	{
		public readonly AttackMoveG2Info Info;
		...
		public AttackMoveG2(Actor self, AttackMoveG2Info info)
		{
		...
		string IOrderVoice.VoicePhraseForOrder(Actor self, Order order)
		{
			...
			if (order.OrderString == "AttackMoveG2" || order.OrderString == "AssaultMove")
				return Info.Voice;
	...
	{
		readonly TraitPair<AttackMoveG2>[] subjects;
		...
		public AttackMoveOrderGenerator(IEnumerable<Actor> subjects, MouseButton button)
		{
			...
			this.subjects = subjects.Where(a => !a.IsDead)
				.SelectMany(a => a.TraitsImplementing<AttackMoveG2>()
					.Select(am => new TraitPair<AttackMoveG2>(a, am)))
				.ToArray();
		}
		...
		protected virtual IEnumerable<Order> OrderInner(World world, CPos cell, MouseInput mi)
		{
			if (mi.Button == expectedButton)
			{
				...
				var orderName = mi.Modifiers.HasModifier(Modifiers.Ctrl) ? "AssaultMove" : "AttackMoveG2";
