...
{
	public class AircraftG3Info : AircraftInfo, ITraitInfo, IPositionableInfo, IFacingInfo, IMoveInfo, ICruiseAltitudeInfo,
		IActorPreviewInitInfo, IEditorActorOptions
	{
		...
		new public bool CanEnterCell(World world, Actor self, CPos cell, Actor ignoreActor = null, bool checkTransientActors = true)
		{
	...
	public class AircraftG3 : Aircraft, ITick, ISync, IFacing, IPositionable, IMove, IIssueOrder, IResolveOrder, IOrderVoice, IDeathActorInitModifier,
		...
		IActorPreviewInitModifier, IIssueDeployOrder, IObservesVariables
	{
		...
		public AircraftG3(ActorInitializer init, AircraftInfo info) : base(init,info)
		{
			Info = info;
			self = init.Self;
		...
		new public virtual IEnumerable<VariableObserver> GetVariableObservers()
		{
		...
		new protected virtual void AddedToWorld(Actor self)
		{
		...
		new protected virtual void RemovedFromWorld(Actor self)
		{
			UnReserve();
		...
		new protected virtual void Tick(Actor self)
		{
			if (firstTick)
			{
				...
				// TODO: AircraftG3 husks don't properly unreserve.
		...
		new public virtual WVec GetRepulsionForce()
		{
			if (!Info.Repulsable)
				return WVec.Zero;
			...
			foreach (var actor in self.World.FindActorsInCircle(self.CenterPosition, Info.IdealSeparation))
			{
				...
				var ai = actor.Info.TraitInfoOrDefault<AircraftG3Info>();
		...
		new public WVec GetRepulsionForce(Actor other)
		{
		...
		new public Actor GetActorBelow()
		{
			// Map.DistanceAboveTerrain(WPos pos) is called directly because AircraftG3 is an IPositionable trait
		...
		new protected void ReserveSpawnBuilding()
		{
		...
		//new public void MakeReservation(Actor target)
		//{
		//	UnReserve();
		...
		//new public void AllowYieldingReservation()
		//{
		//	if (reservation == null)
		//		return;
		...
		//new public WVec FlyStep(int facing)
		//{
		...
		new public bool CanLand(CPos cell)
		{
		...
		//{
		//	// HACK: Work around AttackMoveG2 relying on INotifyIdle.TickIdle to continue its path
		...
		new public bool CanExistInCell(CPos cell) { return true; }
		new public bool IsLeavingCell(CPos location, SubCell subCell = SubCell.Any) { return false; } // TODO: Handle landing
		new public bool CanEnterCell(CPos cell, Actor ignoreActor = null, bool checkTransientActors = true) { return true; }
		new public SubCell GetValidSubCell(SubCell preferred) { return SubCell.Invalid; }
		new public SubCell GetAvailableSubCell(CPos a, SubCell preferredSubCell = SubCell.Any, Actor ignoreActor = null, bool checkTransientActors = true)
		{
			// Does not use any subcell
			return SubCell.Invalid;
		}

		new public void SetVisualPosition(Actor self, WPos pos) { SetPosition(self, pos); }

		// Changes position, but not altitude
		new public void SetPosition(Actor self, CPos cell, SubCell subCell = SubCell.Any)
		{
		...
		new public int EstimatedMoveDuration(Actor self, WPos fromPos, WPos toPos)
		{
			var speed = MovementSpeed;
			return speed > 0 ? (toPos - fromPos).Length / speed : 0;
		}

		new public CPos NearestMoveableCell(CPos cell) { return cell; }

		new public bool IsMoving { get { return isMoving; } set { } }

		new public bool IsMovingVertically { get { return isMovingVertically; } set { } }

		new public bool CanEnterTargetNow(Actor self, Target target)
		{
		...
		new public IEnumerable<IOrderTargeter> Orders
		{
			get
			{
		...
		new public Order IssueOrder(Actor self, IOrderTargeter order, Target target, bool queued)
		{
		...
		new public string VoicePhraseForOrder(Actor self, Order order)
		{
		...
		new public void ResolveOrder(Actor self, Order order)
		{
			...
			else if (order.OrderString == "Enter" || order.OrderString == "Repair")
			{
				...
				if (!ReservableG2.IsAvailableFor(targetActor, self))
				{
					if (!Info.CanHover)
